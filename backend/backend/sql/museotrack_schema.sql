-- Adding IF NOT EXISTS prevents SQL errors when a table is already created

-- Defines the MUSEUM table with a unique Address and required Name
CREATE TABLE IF NOT EXISTS MUSEUM (
    Address VARCHAR(255) PRIMARY KEY,
    Name VARCHAR(255) NOT NULL
);

-- Stores EVENT info. Linked to a museum by Address
CREATE TABLE IF NOT EXISTS EVENT (
    EvID INT PRIMARY KEY,
    Name VARCHAR(255) NOT NULL,
    Start_Date VARCHAR(255) NOT NULL,
    End_Date VARCHAR(255) NOT NULL,
    Address VARCHAR(255),
    FOREIGN KEY(Address) REFERENCES MUSEUM(Address) ON DELETE CASCADE   
);

-- Basic ARTIST profile, includes optional middle name and birth date
CREATE TABLE IF NOT EXISTS ARTIST (
    AID INT PRIMARY KEY,
    Date_of_Birth VARCHAR(255),
    First_Name VARCHAR(255) NOT NULL,
    Middle_Name VARCHAR(255),
    Last_Name VARCHAR(255) NOT NULL
);

-- EXHIBIT is linked to a museum via Address
CREATE TABLE IF NOT EXISTS EXHIBIT (
    ExID INT PRIMARY KEY,
    Name VARCHAR(255) NOT NULL,
    Address VARCHAR(255),
    FOREIGN KEY(Address) REFERENCES MUSEUM(Address) ON DELETE CASCADE
);

-- Base USER table for all types (supervisor, employee, visitor)
CREATE TABLE IF NOT EXISTS USER (
    Email VARCHAR(255) PRIMARY KEY,
    First_Name VARCHAR(255) NOT NULL,
    Middle_Name VARCHAR(255),
    Last_Name VARCHAR(255) NOT NULL,
    Username VARCHAR(255) NOT NULL,
    Password VARCHAR(255) NOT NULL,
    Year_of_Birth INT
);

-- SUPERVISOR is a specialized USER linked to one MUSEUM
CREATE TABLE IF NOT EXISTS SUPERVISOR (
    SEmail VARCHAR(255),
    MuseumAddress VARCHAR(255),
    PRIMARY KEY(SEmail),
    FOREIGN KEY(SEmail) REFERENCES USER(Email) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(MuseumAddress) REFERENCES MUSEUM(Address)
);

-- ARTIFACT contains metadata and is linked to an EXHIBIT and SUPERVISOR
CREATE TABLE IF NOT EXISTS ARTIFACT (
    ArtID INT PRIMARY KEY,
    Description VARCHAR(255),
    Year_Made VARCHAR(255),
    Display_Status VARCHAR(255),
    Name VARCHAR(255) NOT NULL,
    SEmail VARCHAR(255) NOT NULL,
    ExID INT NOT NULL,
    FOREIGN KEY(SEmail) REFERENCES SUPERVISOR(SEmail) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(ExID) REFERENCES EXHIBIT(ExID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Allows custom string-based creators (like organizations) for artifacts
CREATE TABLE IF NOT EXISTS ARTIFACT_CREATORS (
    ArtID INT,
    Creators VARCHAR(255),
    PRIMARY KEY(ArtID, Creators),
    FOREIGN KEY(ArtID) REFERENCES ARTIFACT(ArtID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Standard many-to-many table for artist-artifact relationships
CREATE TABLE IF NOT EXISTS CREATES (
    ArtID INT,
    AID INT,
    PRIMARY KEY(ArtID, AID),
    FOREIGN KEY(ArtID) REFERENCES ARTIFACT(ArtID) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(AID) REFERENCES ARTIST(AID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- EMPLOYEE is a USER that works under a SUPERVISOR and at a MUSEUM
CREATE TABLE IF NOT EXISTS EMPLOYEE (
    EEmail VARCHAR(255),
    SEmail VARCHAR(255),
    MuseumAddress VARCHAR(255),
    PRIMARY KEY(EEmail),
    FOREIGN KEY(EEmail) REFERENCES USER(Email) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(SEmail) REFERENCES SUPERVISOR(SEmail) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(MuseumAddress) REFERENCES MUSEUM(Address)
);

-- VISITOR is a user with limited access to view/review content
CREATE TABLE IF NOT EXISTS VISITOR (
    VEmail VARCHAR(255),
    PRIMARY KEY(VEmail),
    FOREIGN KEY(VEmail) REFERENCES USER(Email) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Logs edits to ARTIFACTS by EMPLOYEES
-- CURRENT_TIMESTAMP automatically sets the date and time when a row is inserted.
CREATE TABLE IF NOT EXISTS EDITS_ARTIFACTS (
    EditID INT AUTO_INCREMENT PRIMARY KEY,
    EEmail VARCHAR(255),
    ArtID INT,
    EditTime DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY(EEmail) REFERENCES EMPLOYEE(EEmail) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(ArtID) REFERENCES ARTIFACT(ArtID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Logs edits to EVENTS by EMPLOYEES
CREATE TABLE IF NOT EXISTS EDITS_EVENTS (
    EditID INT AUTO_INCREMENT PRIMARY KEY,
    EEmail VARCHAR(255),
    EvID INT,
    EditTime DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY(EEmail) REFERENCES EMPLOYEE(EEmail) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(EvID) REFERENCES EVENT(EvID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Logs edits to EXHIBITS by EMPLOYEES
CREATE TABLE IF NOT EXISTS EDITS_EXHIBITS (
    EditID INT AUTO_INCREMENT PRIMARY KEY,
    EEmail VARCHAR(255),
    ExID INT,
    EditTime DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY(EEmail) REFERENCES EMPLOYEE(EEmail) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(ExID) REFERENCES EXHIBIT(ExID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Stores visitor reviews for EVENTS
CREATE TABLE IF NOT EXISTS REVIEW_EVENT (
    VEmail VARCHAR(255),
    EvID INT,
    Review_Desc VARCHAR(255),
    Rating INT,
    PRIMARY KEY(VEmail, EvID),
    FOREIGN KEY(VEmail) REFERENCES VISITOR(VEmail) ON UPDATE CASCADE,
    FOREIGN KEY(EvID) REFERENCES EVENT(EvID) ON UPDATE CASCADE
);

-- Stores visitor reviews for ARTIFACTS
CREATE TABLE IF NOT EXISTS REVIEW_ARTIFACT (
    VEmail VARCHAR(255),
    ArtID INT,
    Review_Desc VARCHAR(255),
    Rating INT,
    PRIMARY KEY(VEmail, ArtID),
    FOREIGN KEY(VEmail) REFERENCES VISITOR(VEmail) ON UPDATE CASCADE,
    FOREIGN KEY(ArtID) REFERENCES ARTIFACT(ArtID) ON UPDATE CASCADE
);a

-- EVENT to EXHIBIT relationship (many-to-many)
CREATE TABLE IF NOT EXISTS PART_OF (
    EvID INT,
    ExID INT,
    PRIMARY KEY(EvID, ExID),
    FOREIGN KEY(EvID) REFERENCES EVENT(EvID) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(ExID) REFERENCES EXHIBIT(ExID) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Tracks which museums a VISITOR has visited
CREATE TABLE IF NOT EXISTS VISITS (
    VEmail VARCHAR(255),
    Address VARCHAR(255),
    PRIMARY KEY(VEmail, Address),
    FOREIGN KEY(VEmail) REFERENCES VISITOR(VEmail) ON UPDATE CASCADE,
    FOREIGN KEY(Address) REFERENCES MUSEUM(Address) ON UPDATE CASCADE
);
